"""Scrape Zurich startups from Seedtable, Failory, and F6S."""
import sys, json, re
...
def scrape_failory(page):
    print("=== FAILORY ZURICH ===")
    page.goto("https://www.failory.com/startups/zurich", timeout=30000)
...
def scrape_seedtable(page):
    print("=== SEEDTABLE ZURICH ===")
    page.goto("https://www.seedtable.com/best-startups-in-zurich", timeout=30000)
    page.wait_for_timeout(5000)
    for _ in range(15):
        page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
        page.wait_for_timeout(1000)
...
def scrape_f6s(page):
    print("=== F6S ZURICH ===")
    page.goto("https://www.f6s.com/companies/switzerland/zurich/lo", timeout=30000)
    page.wait_for_timeout(5000)
    ...
    # Profile lookups for missing websites
    bot_blocked = False
...
if __name__ == "__main__":
    ...
    with open("failory_zurich.json", "w", encoding="utf-8") as f:
        json.dump(failory, f, ensure_ascii=False, indent=2)
    with open("seedtable_zurich.json", "w", encoding="utf-8") as f:
        json.dump(seedtable, f, ensure_ascii=False, indent=2)
    with open("f6s_zurich.json", "w", encoding="utf-8") as f:
        json.dump(f6s, f, ensure_ascii=False, indent=2)

    print(f"\nTotal raw: Failory={len(failory)}, Seedtable={len(seedtable)}, F6S={len(f6s)}")
    print(f"Sum: {len(failory) + len(seedtable) + len(f6s)}")
