"""Merge Silicon Valley data and add tab to workbook."""
...
def normalize_name(name):
    n = name.lower().strip()
    n = re.sub(r"\s*\(.*?\)", "", n)
    n = re.sub(r"\s*(ltd\.?|inc\.?|co\.?|corp\.?|pte\.?|ag|gmbh|sa|sas|sl|llc|corporation|technologies|tech|security|ai)\s*\.?$", "", n, flags=re.IGNORECASE)
...
sources = []
for fname in ["failory_sf_siliconvalley.json", "failory_sj_siliconvalley.json",
              "seedtable_sf_siliconvalley.json", "seedtable_sj_siliconvalley.json",
              "f6s_sf_siliconvalley.json", "f6s_sj_siliconvalley.json"]:
...
for candidate in ["global_startups_final_v2.xlsx", "global_startups_final_new.xlsx", "global_startups_final.xlsx"]:
    if os.path.exists(candidate):
        src = candidate
        break
wb = load_workbook(src)

if "Silicon Valley" in wb.sheetnames:
    del wb["Silicon Valley"]

ws = wb.create_sheet("Silicon Valley")
...
print(f"Silicon Valley tab added: {len(df)} startups")
