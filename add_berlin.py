"""Merge Berlin data and add tab to global_startups_final.xlsx."""
import json, re, sys
...
with open("failory_berlin.json", encoding="utf-8") as f:
    failory = json.load(f)
with open("seedtable_berlin.json", encoding="utf-8") as f:
    seedtable = json.load(f)
with open("f6s_berlin.json", encoding="utf-8") as f:
    f6s = json.load(f)
...
if "Berlin" in wb.sheetnames:
    del wb["Berlin"]

ws = wb.create_sheet("Berlin")
...
try:
    ...
    wb.save(outfile)
    print(f"(original file locked - saved as {outfile}, close Excel and rename)")
print(f"Berlin tab added: {len(df)} startups")
