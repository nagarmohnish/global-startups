"""Merge Zurich data and add tab to global_startups_final.xlsx."""
import json, re, sys
...
with open("failory_zurich.json", encoding="utf-8") as f:
    failory = json.load(f)
with open("seedtable_zurich.json", encoding="utf-8") as f:
    seedtable = json.load(f)
with open("f6s_zurich.json", encoding="utf-8") as f:
    f6s = json.load(f)
...
if "Zurich" in wb.sheetnames:
    del wb["Zurich"]

ws = wb.create_sheet("Zurich")
...
print(f"Zurich tab added: {len(df)} startups")
