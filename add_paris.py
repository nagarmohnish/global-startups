"""Merge Paris data and add tab to global_startups_final.xlsx."""
import json, re, sys
...
def normalize_name(name):
    n = name.lower().strip()
    n = re.sub(r"\s*\(.*?\)", "", n)
    n = re.sub(r"\s*(ltd\.?|inc\.?|co\.?|corp\.?|pte\.?|sas|sa|corporation|technologies|tech|security|ai)\s*\.?$", "", n, flags=re.IGNORECASE)
...
with open("failory_paris.json", encoding="utf-8") as f:
    failory = json.load(f)
with open("seedtable_paris.json", encoding="utf-8") as f:
    seedtable = json.load(f)

merged = merge_deduplicate([failory, seedtable])
print(f"Failory: {len(failory)}, Seedtable: {len(seedtable)}, Merged: {len(merged)}")
...
wb = load_workbook("global_startups_final.xlsx")
if "Paris" in wb.sheetnames:
    del wb["Paris"]

from openpyxl.utils.dataframe import dataframe_to_rows
ws = wb.create_sheet("Paris")
for r_idx, row in enumerate(dataframe_to_rows(df, index=False, header=True), 1):
    for c_idx, value in enumerate(row, 1):
        ws.cell(row=r_idx, column=c_idx, value=value)

format_sheet(ws, df)
wb.save("global_startups_final.xlsx")
print(f"Paris tab added: {len(df)} startups")
