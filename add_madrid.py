"""Merge Madrid data and add tab to global_startups_final.xlsx."""
...
def normalize_name(name):
    n = name.lower().strip()
    n = re.sub(r"\s*\(.*?\)", "", n)
    n = re.sub(r"\s*(ltd\.?|inc\.?|co\.?|corp\.?|pte\.?|ag|gmbh|sa|sas|sl|corporation|technologies|tech|security|ai)\s*\.?$", "", n, flags=re.IGNORECASE)
...
with open("failory_madrid.json", encoding="utf-8") as f:
    failory = json.load(f)
with open("seedtable_madrid.json", encoding="utf-8") as f:
    seedtable = json.load(f)
with open("f6s_madrid.json", encoding="utf-8") as f:
    f6s = json.load(f)
...
if "Madrid" in wb.sheetnames:
    del wb["Madrid"]

ws = wb.create_sheet("Madrid")
...
print(f"Madrid tab added: {len(df)} startups")
