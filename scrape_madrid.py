"""Scrape Madrid startups from Failory, Seedtable, and F6S."""
import sys, json, re
...
def scrape_failory(page):
    print("=== FAILORY MADRID ===")
    page.goto("https://www.failory.com/startups/madrid", timeout=30000)
...
def scrape_seedtable(page):
    print("=== SEEDTABLE MADRID ===")
    page.goto("https://www.seedtable.com/best-startups-in-madrid", timeout=30000)
    page.wait_for_timeout(5000)
    for _ in range(15):
        page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
        page.wait_for_timeout(1000)
...
def scrape_f6s(page):
    print("=== F6S MADRID ===")
    page.goto("https://www.f6s.com/companies/spain/madrid/lo", timeout=30000)
    page.wait_for_timeout(5000)
...
if __name__ == "__main__":
    ...
    with open("failory_madrid.json", "w", encoding="utf-8") as f:
        json.dump(failory, f, ensure_ascii=False, indent=2)
    with open("seedtable_madrid.json", "w", encoding="utf-8") as f:
        json.dump(seedtable, f, ensure_ascii=False, indent=2)
    with open("f6s_madrid.json", "w", encoding="utf-8") as f:
