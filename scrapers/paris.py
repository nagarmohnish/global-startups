"""Scrape Paris startups from Failory and Seedtable."""
import sys, json, re
...
def scrape_failory(page):
    print("=== FAILORY PARIS ===")
    page.goto("https://www.failory.com/startups/paris", timeout=30000)
...
def scrape_seedtable(page):
    print("=== SEEDTABLE PARIS ===")
    page.goto("https://www.seedtable.com/best-startups-in-paris", timeout=30000)
    page.wait_for_timeout(5000)
    for _ in range(15):
        page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
        page.wait_for_timeout(1000)
...
if __name__ == "__main__":
    ...
    with open("failory_paris.json", "w", encoding="utf-8") as f:
        json.dump(failory, f, ensure_ascii=False, indent=2)
    with open("seedtable_paris.json", "w", encoding="utf-8") as f:
        json.dump(seedtable, f, ensure_ascii=False, indent=2)

    print(f"\nTotal raw: Failory={len(failory)}, Seedtable={len(seedtable)}")
    print(f"Sum: {len(failory) + len(seedtable)}")
