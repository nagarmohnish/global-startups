"""Scrape Berlin startups from Seedtable, Failory, and F6S."""
import sys, json, re
...
def scrape_failory(page):
    print("=== FAILORY BERLIN ===")
    page.goto("https://www.failory.com/startups/berlin", timeout=30000)
...
def scrape_seedtable(page):
    print("=== SEEDTABLE BERLIN ===")
    page.goto("https://www.seedtable.com/best-startups-in-berlin", timeout=30000)
    page.wait_for_timeout(5000)
    for _ in range(15):
        page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
        page.wait_for_timeout(1000)
...
def scrape_f6s(page):
    print("=== F6S BERLIN ===")
    page.goto("https://www.f6s.com/companies/germany/berlin/lo", timeout=30000)
    page.wait_for_timeout(5000)
...
if __name__ == "__main__":
    ...
    with open("failory_berlin.json", "w", encoding="utf-8") as f:
        json.dump(failory, f, ensure_ascii=False, indent=2)
    with open("seedtable_berlin.json", "w", encoding="utf-8") as f:
        json.dump(seedtable, f, ensure_ascii=False, indent=2)
    with open("f6s_berlin.json", "w", encoding="utf-8") as f:
